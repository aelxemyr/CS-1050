...
public class DiceGame
{
    ...
    public void play()
    {
        ...
        while (playing && player.getStake() > 0) {
            placeBet();
            executeRound();
    ...
    {
        String betString = ui.requestString("You have " + this.player.getStake() + " tokens.\n" +
                                            "How much would you like to bet?");
        Integer bet = parseInt(betString);
        if (bet == null || bet > player.getStake() || bet < 0) {
            ui.showMessage("Please enter a nonnegative integer amount.");
            placeBet();
        }
        else {
            costOfPlay = bet;
